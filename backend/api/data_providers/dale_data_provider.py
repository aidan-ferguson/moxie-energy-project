...
class DALEDataProvider(DataProvider):
    ...
    def get_energy_data(house):
        # Get the start and end timestamps of the dataset, we need to take the maximum start date and minimum end date
        #   in order to encapsulate all the data
        with open(os.path.join(os.path.join(DALE_FOLDER, house), "start-end.dat"), "r") as file:
            data = file.readlines()
        DALE_START_DATE = int(data[0])
        DALE_END_DATE = int(data[1])
        
        # Initial energy usage
        start_time = DALE_START_DATE
        start_time = datetime.datetime.fromtimestamp(start_time)
        end_time = start_time + relativedelta(weeks=4)
        initial_usage = DALEDataProvider.read_dale_data(house, start_time, end_time)
        ...
        curr_day = DALEDataProvider.read_dale_data(house, start_time, end_time)
        ...
        prev_week = DALEDataProvider.read_dale_data(house, start_time, end_time)
